...
class RollingNumberTextView : TextView {
    ...
    private fun init() {
        ...
        speed = Array(textArray.size, { i -> 12 - i })
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (i in 0 until textArray.size)
            for (j in 0 until columns) {
                ...
                if (i == 0 &&j == columns -1&& !finishedColumn[0])
                    Log.i(TAG, "onDraw:column:$i row:$j  baseLine:$baseLine   base $base  text:$text")
    ...
    private fun checkColumnFinished(columnIndex: Int, rowIndex: Int): Boolean {
        ...
        //so this column should stop rolling
        if (rowIndex == columns - 1 && baseLine + rowIndex * measuredHeight + rolledOffset[columnIndex] <= baseLine) {
    ...
    /**01234569012345690
     *     456789012345690
     *we calculate from tail to get the number,the calculation will be quite easy and quick
     *
     */
    private fun getNumberText(origin: Int, rowIndex: Int): String {
        val r = origin - (columns - rowIndex)
        return if (r >= 0) r.toString() else (r % 10 + 10).toString()
    }
